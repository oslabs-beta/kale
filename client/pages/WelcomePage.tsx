...
export default function WelcomePage() {
  ...
  // Assuming useGrabMetricsMutation is a hook from RTK Query
  const [grabMetrics, { data: metricsData, error: metricsError }] =
    useGrabMetricsMutation();

  function handleClick(urlString: string) {
    try {
      console.log(urlString);
      grabMetrics(urlString)
        .unwrap()
        .then((response) => {
          console.log('data created!', response);
        })
      ...
      <div className='relative isolate px-6 pt-14 lg:px-8'>
        ...
        <div className='h-screen mx-auto max-w-3xl py-28 sm:py-32 lg:py-20'>
          <div className='text-center'>
            <h1 className='text-4xl font-bold tracking-tight text-zinc-200 sm:text-6xl'>
              Welcome to{' '}
              <span className='inline-block text-kalegreen-400'>kale</span>,
              your new favorite Kubernetes autoscaling tool
            </h1>
            <p className='mt-6 text-lg leading-8 text-zinc-400'>
              Anim aute id magna aliqua ad ad non deserunt sunt. Qui irure qui
